...
const { scanDir, ocr } = require('../dist')
...
test.serial('scanDir', async t => {
  ...
  // t.deepEqual(res.matched.get(pdf).matches, expected.split(' '))
  ...
  // t.true(res.matched.get(pdf).text.includes(expected))
...
test.serial('scanDir should save progress and log file', async t => {
  ...
  t.is(progress.visited.length, 7)
